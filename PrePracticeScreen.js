...
function PrePracticeScreen() {
  ...
  const { selectedPrompt, categoryPrompts, isFromNextPrompt } =
    ...
    useCallback(() => {
      if (!selectedPrompt) {
        ...
        if (navigation.canGoBack()) navigation.goBack();
          ...
          navigation.navigate("PracticeTab", { screen: "CategorySelection" }); // Fallback
        return;
      }

      console.log(
        `PrePracticeScreen focused/prompt changed: ${selectedPrompt.title}. Resetting countdown.`
      );
      ...
      timerRef.current = setInterval(() => {
        setCountdown((prevCountdown) => {
          if (prevCountdown <= 1) {
            // When it reaches 1, next tick will be 0
      ...
      return () => {
        console.log(
          `PrePracticeScreen unfocused/cleanup: ${selectedPrompt.title}`
        );
        clearInterval(timerRef.current);
      };
    ...
    if (countdown === 0) {
      ...
      (async () => {
        if (selectedPrompt && categoryPrompts) {
          ...
          console.error(
            "PrePracticeScreen: Countdown finished but prompt data missing for navigation."
          );
          if (navigation.canGoBack()) navigation.goBack();
          else
            navigation.navigate("PracticeTab", { screen: "CategorySelection" }); // Fallback
  ...
  const handleSkip = () => {
    ...
    if (selectedPrompt && categoryPrompts) {
      ...
      console.error(
        "PrePracticeScreen: Skip pressed but prompt data missing for navigation."
      );
      navigation.goBack();
    }
  };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.promptInfoContainer}>
          ...
          <Text style={styles.promptTitle}>
            {selectedPrompt.title || "Unnamed Prompt"}
          </Text>
          <Text style={styles.promptSummaryLabel}>
            You'll be speaking about:
          </Text>
