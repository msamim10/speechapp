...
function TeleprompterScreen({ route, navigation }) {
  ...
  const recordPracticeSession = useCallback(async () => {
    ...
    try {
      ...
      // Increment play count for this prompt
      if (promptId && promptId !== "unknown") {
        const playCountsJson = await AsyncStorage.getItem(PROMPT_PLAY_COUNTS_KEY);
        let playCounts = playCountsJson ? JSON.parse(playCountsJson) : {};
        playCounts[promptId] = (playCounts[promptId] || 0) + 1;
        await AsyncStorage.setItem(PROMPT_PLAY_COUNTS_KEY, JSON.stringify(playCounts));
        console.log(`Play count for prompt ${promptId} updated to ${playCounts[promptId]}`);
      }

    } catch (error) {
      console.error("Failed to record practice session or update play count:", error);
    }
